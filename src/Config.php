...
abstract class Xapp_Config implements Xapp_Config_Interface
{
     ...
     * value in case key is not found in config. the third argument can also be a throwable exception. the fourth argument
     * if set to boolean true will return the config value raw without to object conversion
     *
     * @error 12404
     ...
     * @param bool $raw expects boolean value for returning config as raw value
     * @return null|mixed
     * @throws Exception
     */
    public static function retrieve($ns, $key = null, $default = null, $raw = false)
    {
        ...
        if($ns === 'global')
        {
            if((bool)$raw)
            {
                return self::out(self::$_globalConfig, $key, $default, $cache);
            }else{
            ...
            if((bool)$raw)
            {
                return self::out(self::$_instances[$ns]->_config, $key, $default, $cache);
            }else{
    ...
    /**
     * get config or config value if key is set. returns default value if key is not found. if third argument is set to
     * boolean true will return config value raw without object conversion
     *
     * @error 12408
    ...
    public function get($key = null, $default = null, $raw = false)
    {
        if($key !== null)
        {
            if((bool)$raw)
            {
                return self::out($this->_config, $key, $default, $this->_cache);
            }else{
     ...
     */
    protected static function toArray($data = null)
    {
        if(is_object($data))
        {
